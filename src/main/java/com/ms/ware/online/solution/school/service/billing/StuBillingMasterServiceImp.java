...
public class StuBillingMasterServiceImp implements StuBillingMasterService {
    ...
    public Object getAll(String billNo) {
        ...
        sql = "SELECT BILL_NO billNo,IFNULL(REG_NO,'N/A') regNo,GET_BS_DATE(B.ENTER_DATE) enterDate,B.ENTER_BY enterBy,STUDENT_NAME studentName,FATHER_NAME fatherName,MOBILE_NO mobileNo,ADDRESS address,P.NAME program,C.NAME 'class',IFNULL(B.REMARK,'') remark,'' rollNo,G.`NAME` subjectGroup FROM stu_billing_master B,program_master P,class_master C,subject_group G WHERE REG_NO IS NULL AND B.PROGRAM=P.ID AND B.CLASS_ID=C.ID AND G.`ID`=B.`SUBJECT_GROPU` AND BILL_NO='" + billNo + "' "
                + " UNION "
                + "SELECT BILL_NO billNo, IFNULL(REG_NO, 'N/A') regNo, GET_BS_DATE(B.ENTER_DATE) enterDate, B.ENTER_BY enterBy, S.STU_NAME studentName, S.guardians_name fatherName, S.MOBILE_NO mobileNo, CONCAT(DISTRICT, ' ', MUNICIPAL, ' ', WARD_NO, ' ', TOL) address, P.NAME program, case when B.class_id = S.class_id then CONCAT(C.NAME, ' ', section) else C.NAME end 'class', IFNULL(B.REMARK, '') remark, S.ROLL_NO rollNo, G.`NAME` subjectGroup FROM stu_billing_master B join student_info S on B.REG_NO = S.ID join program_master P on B.PROGRAM = P.ID join class_master C on B.CLASS_ID = C.ID join subject_group G on B.`SUBJECT_GROPU` = G.`ID` WHERE BILL_NO='" + billNo + "'";
