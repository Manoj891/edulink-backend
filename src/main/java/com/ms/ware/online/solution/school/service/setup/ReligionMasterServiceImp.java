...
import com.ms.ware.online.solution.school.dao.setup.ReligionMasterDao;
...
@Service
public class ReligionMasterServiceImp implements ReligionMasterService {
    @Autowired
    ...
    public Object getAll() {
        return da.getAll("from ReligionMaster order by id");
    }
    ...
    public Object save(ReligionMaster obj) {
        ...
        try {
            try {
                sql = "SELECT ifnull(MAX(ID),0)+1 AS id FROM religion_master";
    ...
    public Object delete(String id) {
        ...
        id = "'" + id.replace(",", "','") + "'";
        sql = "DELETE FROM religion_master WHERE ID IN (" + id + ")";
        row = da.delete(sql);
        msg = da.getMsg();
        if (row > 0) {
            return message.respondWithMessage("Success");
