...
public class SubjectGroupRestController {
    ...
    public Object indexClassWise(@RequestParam long program, @RequestParam long classId, @RequestParam long subjectGroup) {
        return db.getRecord("select s.id id,concat(s.name,' (',d.subject_code,')') AS name from subject_group_detail d,subject_master s where s.id=d.subject and program=" + program + " AND class_id=" + classId + " AND subject_group=" + subjectGroup);
    }
